...
import org.csl.util.SToCThread;
...
//		this.LOGGER = new SToCALogger(this.getParameter("do.stoca.logging"));
		...
		if (this.HTML_WINDOW == null) {
			this.logp("JSObject.getWindow() fails.");
		}

		this.STOC_THREAD =
			SToCThread.startTheThread(this.JA_OPTS, this.LOGGER);
		this.logb(STOCA_PREFIX+": init() done.");
	}
 ...
 */
	...
	private void doTerminate(final String MTHD) {
		...
		this.logp("Quit delay (ms): "+qdelayms);
 ...
 */
	public void doSiGMLURLToCAS(
		...
		try {
			this.logp("sigmlURLToCAS() for "+avatar);
			this.logp("URL: "+sigmlurl);
 ...
 */
	public void doSiGMLTextToCAS(
		...
		try {
			this.logp("sigmlTextToCAS() for "+avatar);
			if (sigml == null) { this.logp("SiGML is null."); }
			else { this.logp("SiGML length="+sigml.length()+"."); }
