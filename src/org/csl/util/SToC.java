...
import static org.csl.SToCApplet.JSON_FORMAT;
import static org.csl.SToCApplet.XML_FORMAT;
...
/** Logger */
	...
	public void sigmlURLToCAS(
		...
		if (casfmt.equalsIgnoreCase(JSON_FORMAT)) {
			final JSONStreamGenFromAnim JSGEN =	new JSONStreamGenFromAnim(avatar, sa,  this.LOGGER);
		}
			...
			this.logp("Unknown CAS result format tag: "+casfmt+".");
		}
	}

	public void sigmlTextToCAS(String sigml, String avatar, String casfmt, CASDataReceiver casrcvr) {
		...
		if (casfmt.equalsIgnoreCase(JSON_FORMAT)) {
			final JSONStreamGenFromAnim JSGEN =
				new JSONStreamGenFromAnim(avatar, sa, this.LOGGER);
		}
		else if (casfmt.equalsIgnoreCase(XML_FORMAT)) {
			// JRWG: Ought to handle null SiGMLAnimation
			cas = this.animToCASString(avatar, sa);
	...
	public void sigmlTextToJSON(String sigml, String avatar) {
	...
	protected SiGMLAnimation sigmlURLToAnim(String sigmlurl, String avatar) {
		...
		if (this.JA_OPTS.getAvatarsEnv().isValidAvatar(avatar)) {
			...
			this.logp("Invalid avatar for SiGML URL: "+avatar);
			return null;
		}
	}
	...
	//	if (this.JA_OPTS.getAvatarsEnv().isValidAvatar(avatar)) {
			...
			final StreamedAnimationLoader SA_LOADER =new StreamedAnimationLoader(avatar, this.JA_OPTS, SIGML_RDR, null);
			SA_LOADER.processSiGML();
			return SA_LOADER.getAnimation();

//		} else {
//			this.logp("Invalid avatar for SiGML Text: "+avatar);
//			return null;
//		}
	}
