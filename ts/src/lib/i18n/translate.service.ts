...
export class TranslateService {
     ...
     * @param lang
     */
    public load(lang: string,dir:string ): void {
     ...
     * @returns {Observable<*>}
     */
    private retrieveTranslations(lang: string,dir?:string): Observable<any> {
    ...
    public getTranslation(lang: string,dir?:string): Observable<any> {
        ...
        if(isDefined(dir)){
          this.loadingTranslations.take(1)
              .subscribe((res: any) => {
                  if(typeof res.json==='function') { res=res.json(); }
                  this.translations[lang]=Object.assign(this.translations[lang],res);
                  this.updateLangs();
                  this.pending = false;
    ...
    public getParsedResult(translations: any, key: any, interpolateParams?: Object): any {
        ...
        if(key instanceof Array) {
            ...
            if(observables) {
                let mergedObs: any;
                for(const k of key) {
                    const obs = typeof result[k].subscribe === 'function' ? result[k] : Observable.of(result[k]);
